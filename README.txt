...
bookSeats(uint[] seatNumbers): 
                    This function should take an array of seat numbers as input. 
 ...
 showAvailableSeats() returns (uint[]):
                     This function should return the array of all the seat numbers that are available.
                     The order of the seat number can be anything.

checkAvailability(uint seatNumber) returns (bool):
                    Using this function, availability of a seat can be checked. 
                    If seat corresponding to seatNumber is available, then the function must return true, else it must return false.

myTickets() returns (uint[]):
